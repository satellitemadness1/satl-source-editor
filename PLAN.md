...
**Read the repo's `pushed_at` before trusting anything below.** This is a
fast-moving repository: an earlier pass in this same session read a tree
that was five days old and drew wrong conclusions from it, while the user
was pushing *004 revision 06* twenty minutes later. Record the SHA you read.

As of **commit `b56bdc73`, "004 revision 06"** (2026-09-21):

    words/words.tsv        388 lines, the registry, tab-separated
    words/words_004.tsv    24 lines, what 004 adds
    words/satellite_words.hpp, words/make_words.py
    satellite/bytecode/word_codes.hpp   (82 KB, generated)

TSV, so no X-macro parsing is needed — column 1 is the number path, column 2
is the dotted name:

```bash
curl -s https://raw.githubusercontent.com/satellitemadness1/satellite/main/words/words.tsv |
cut -f2
```

**388 names under 27 namespaces**: `variable` 115, `library` 71,
`container` 56, `system` 39, `random` 18, `console` 11, `network` 9,
`file` 8, `directory` 7, `statement` 6, `time` 6, `bool` 4, `include` 4,
`return` 4, `capsule` 3, `feedback` 3, `help` 3, `thread` 3, `window` 3,
plus `constructor`, `infinity`, `main`, `protected`, `public`, `spacesuit`,
`analyze`, `returns`.

`satellite.variable.*` types: binary, bool, capsule, date, duration,
expression, file, float, hex, **infinity**, network, number, **percentage**,
string, thread, time, variant, **window**.

**A correction to an earlier claim in this file.** A previous pass compared
the dictated notes against `old_versions/second_satellite` — the *archived*
interpreter — and reported that the user had misremembered `infinity` and
`window`. That was wrong. Both are real, and documented at length:
`SATELLITE_INFINITY.md` is 107 KB and `SATELLITE_WINDOW.md` is 38 KB, both
at the top level. What the current registry still does NOT contain is
`satellite.statement.switch`, `.case`, `.finally`, or a thread `.stop()` —
those four remain dictated-only, and are worth one question rather than an
assumption.

### Literal syntax the highlighter must handle

From the current tests, which are the corpus to check `satellite.lang`
against — there are now about **90** `.satl` programs under `tests/`,
`examples/`, `test_programs/` and `experiments/`:

| form | example | source |
|---|---|---|
| binary literal | `0b10101010` | `tests/binary_0b.satl` |
| percentage literal | `50%`, `12.5%`, `1000000000000%` | `tests/percentage.satl` |
| long decimals | `33.333333333333333333333333333333335` | `tests/percentage.satl` |
| power, two spellings | `2 ** 3 ** 2` and `2 ^ 3 ^ 2`, right to left | `tests/power_stars.satl` |
| `main` with no arguments | `satellite.capsule satellite.main()` | most 004 tests |
| a `//` inside a string is not a comment | `"a // inside a string is not a comment"` | `examples/hello_world.satl` |

`tests/` also covers negative binaries, binaries written without the `b`,
comments after values, wide characters, chains, and `for` loops with steps —
a ready-made test set for the highlighter, and far more informative than
inventing examples.

Also relevant to item 2 below: the language repo has a branch
**`icons-and-satl-mime-type`**, which is that work already in progress.
